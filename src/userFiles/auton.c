...
char* GenFileName (unsigned short* duration)
{
    if (digitalRead(Jumper_SkillsOrAuton))
    {
        ...
        *duration = 15000; // 15 Seconds
        filename_buf[1] = digitalRead(Jumper_RedOrBlue_And_LeftOrRight) ? 'R' : 'L'; // L: Left, R: Right
    }
    else
    {
        *filename_buf = 'S'; // Skills
        *duration = 60000; // 60 Seconds
        filename_buf[1] = digitalRead(Jumper_RedOrBlue_And_LeftOrRight) ? 'B' : 'R'; // R: Red, B: Blue
    };
    filename_buf[2] = '0' + ((digitalRead(Jumper_version1or0) ? 0 : 1) | (digitalRead(Jumper_version2or0) ? 0 : 2));
    filename_buf[3] = '\0';
    return filename_buf;
};
...
void FollowInstructions ()
{
    ...
    if (!file) ErrorBlinkInfinite(); // Could not open the file, presumably because it doesn't exist.
    fseek(file,0,SEEK_END);
    unsigned int instructioncount;// = ftell(file) / (membersize(Instruction,time) + membersize(Instruction,label) + membersize(Instruction,power));
    //fseek(file,0,SEEK_SET);
    if (fread(&instructioncount,sizeof(instructioncount),1,file) < sizeof(instructioncount)) goto CLOSE_ERRBLINK;
    Instruction* instructions = malloc(sizeof(Instruction) * instructioncount);
    if (!instructions) goto ALLOCFAILURE;
    unsigned int iter = instructioncount;
    Instruction* instrptr = instructions;
    while (iter--)
    {
        if (fread(&(instrptr->time),sizeof(instrptr->time),1,file) < sizeof(instrptr->time)) goto DEALLOC_CLOSE_ERRBLINK;
        if (fread(&(instrptr->label),sizeof(instrptr->label),1,file) < sizeof(instrptr->label)) goto DEALLOC_CLOSE_ERRBLINK;
        if (fread(&(instrptr->power),sizeof(instrptr->power),1,file) < sizeof(instrptr->power)) goto DEALLOC_CLOSE_ERRBLINK;
        instrptr++;
    };
    ...
    while (instructioncount--) // Don't try to read past the end of the file.
    {
        ...
        while (millis() < StartTime + instrptr->time);// wait(20);
