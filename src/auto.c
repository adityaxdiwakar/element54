...
mogoAutonMaster(int coneCount) {
  gyroReset(GYRO_LR1); encoderReset(ENC_RIGHT);

                   //[arm, mogo, drive, bar]  ``
    int armPos = 2000; int mogoPos = 3000; int drivePos = 1200; int barPos = 350;
  arm(127);
  ...
  taskDelete(barMove_1); taskDelete(armMove_1);
...
boonkTheMogo() {
  ...
  driveSpeed(0); encoderReset(ENC_RIGHT);
  driveForward();
  while(encoderGet(ENC_RIGHT) < 1000) {delay(15);}
  driveSpeed(0);
  gyroPos = -45;
  rotate = taskCreate(rotateToPID, TASK_DEFAULT_STACK_SIZE, (void*) gyroPos, TASK_PRIORITY_DEFAULT);
  while (gyroGet(GYRO_LR1) > gyroPos) {
  ...
  wait(500); taskDelete(rotate); encoderReset(ENC_RIGHT);
  driveSpeed(0); driveForward(); 
  while(encoderGet(ENC_RIGHT) < 1000) {delay(15);}
  gyroPos = -180;
  rotate = taskCreate(rotateToPID, TASK_DEFAULT_STACK_SIZE, (void*) gyroPos, TASK_PRIORITY_DEFAULT);
  while (gyroGet(GYRO_LR1) > gyroPos) {
  ...
  wait(500); taskDelete(rotate);
  driveSpeed(0); driveForward();
  wait(250); mogo(127); wait(500);
  mogo(0); driveBackward();
  wait(300);
  driveSpeed(0);
}
