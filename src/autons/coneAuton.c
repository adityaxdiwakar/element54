...
void progSkills() {
  ...
  mobileGoal(0); taskDelete(drop20); driveStop(); wait(300); taskDelete(holdBase); TaskHandle escape = taskRunLoop(driveBackward, 1); wait(1000); driveStop();
  taskDelete(escape);
  //end of the first task lol

  TaskHandle equalizeBase_2 = taskRunLoop(driveForward, 1); wait(250); taskDelete(equalizeBase_2); driveStop();
  TaskHandle surgeBase_2 = taskRunLoop(driveBackward, 1); wait(250); taskDelete(surgeBase_2); driveStop();
  turnRad = 0;
  TaskHandle thirdTurn = taskCreate(posSetTurn, TASK_DEFAULT_STACK_SIZE, (void*)turnRad, TASK_PRIORITY_DEFAULT);
  wait(1000); taskDelete(thirdTurn); driveStop();
}
